...
std::string N3CtrlFSM::state_to_str()
{
    ...
    case AUTO_TAKEOFF : return std::string("AUTO-TAKEOFF");
    case AUTO_LAND : return std::string("AUTO-LAND");
...
bool N3CtrlFSM::toggle_api_mode(bool on_off) {

    djiros::SDKControlAuthority api_cmd;
    if (on_off) {
        api_cmd.request.control_enable = 1;
        if (!(set_api_mode_srv.call(api_cmd) && api_cmd.response.result)) {
            ROS_ERROR("Enter API rejected by N3!");
            return false;
        }
    } else {
        api_cmd.request.control_enable = 0;
        if (!(set_api_mode_srv.call(api_cmd) && api_cmd.response.result)) {
            ROS_ERROR("Exit API rejected by N3!");
            return false;
        }
    }
    return true;
...
bool N3CtrlFSM::toggle_arm_disarm(bool arm) {

    djiros::DroneArmControl arm_cmd;
    arm_cmd.request.arm = 0;
    if (arm) arm_cmd.request.arm = 1;
    if (!(arming_client_srv.call(arm_cmd) && arm_cmd.response.result)) {

        if (arm)
            ROS_ERROR("ARM rejected by N3!");
        else
            ROS_ERROR("DISARM rejected by N3!");
...
void N3CtrlFSM::set_start_pose_for_takeoff_land(const Odom_Data_t &odom) {
...
void N3CtrlFSM::publish_trigger(const nav_msgs::Odometry &odom_msg) {
